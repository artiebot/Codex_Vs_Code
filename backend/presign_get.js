#!/usr/bin/env node
/**
 * Generate a presigned HTTPS GET URL for Cloudflare R2 downloads.
 *
 * Usage:
 *   node backend/presign_get.js --device dev1 --event evt-2025-10-12T07:31:22Z-abc123 --thing clip [--expires 900]
 *
 * Environment variables match presign_put.js (R2_ACCOUNT_ID, R2_BUCKET, etc.).
 */
...
const { key } = buildKey({ deviceId, eventId, thing });
...
function usage(message) {
  ...
  console.error("Usage: node backend/presign_get.js --device dev1 --event evt-... --thing thumb|clip [--expires 900]");
  process.exit(1);
}
