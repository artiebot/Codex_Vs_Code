...
constexpr uint32_t kBootMagic = 0x42544331;
constexpr uint8_t kBootForceCount = 3;
constexpr uint64_t kBootWindowUs = 60ULL * 1000000ULL;
constexpr uint8_t kWifiFailureLimit = 3;
constexpr unsigned long kWifiFailureWindowMs = 60000UL;
...
void Provisioning::begin() {
  gProvisioning = this;
  if (PROVISION_BUTTON_PIN >= 0) {
    pinMode(PROVISION_BUTTON_PIN, INPUT_PULLUP);
  }
  Storage::begin();
  recordBootAttempt();
  load();
  bool buttonHeld = buttonRequestedSetup();
  if (boot_forced_setup_ || buttonHeld || !ready_) {
...
bool Provisioning::buttonRequestedSetup() {
  if (PROVISION_BUTTON_PIN < 0) return false;
...
void Provisioning::enterProvisioningMode() {
  ...
  setup_mode_ = true;
  ready_ = false;
  runtime_triggered_ = true;
  wifi_failures_ = 0;
  wifi_fail_window_start_ms_ = 0;
  wifi_connected_ = false;
  boot_forced_setup_ = false;
  resetBootCounter();
  Topics::init(deviceId());
  startSetupAp();
}
...
void Provisioning::runtimeButtonCheck() {
  if (PROVISION_BUTTON_PIN < 0) return;
...
void Provisioning::recordBootAttempt() {
  BootState state{ kBootMagic, 0, 0 };
  BootState stored{};
  if (Storage::getBytes("prov", "boot", &stored, sizeof(stored)) && stored.magic == kBootMagic) {
    state = stored;
  }
  uint64_t now = esp_timer_get_time();
  if (state.last_us != 0 && (now - state.last_us) < kBootWindowUs) {
    state.count++;
  } else {
    state.count = 1;
  }
  state.last_us = now;
  Storage::setBytes("prov", "boot", &state, sizeof(state));
  if (state.count >= kBootForceCount) {
    boot_forced_setup_ = true;
    state.count = 0;
    Storage::setBytes("prov", "boot", &state, sizeof(state));
  } else {
    boot_forced_setup_ = false;
...
void Provisioning::resetBootCounter() {
  BootState state{ kBootMagic, 0, esp_timer_get_time() };
  Storage::setBytes("prov", "boot", &state, sizeof(state));
}
...
void Provisioning::notifyWifiConnectTimeout() {
  ...
  wifi_connected_ = false;
  unsigned long now = millis();
  if (wifi_failures_ == 0 || (now - wifi_fail_window_start_ms_) > kWifiFailureWindowMs) {
    wifi_failures_ = 1;
    wifi_fail_window_start_ms_ = now;
  } else {
    wifi_failures_++;
  }
  if (wifi_failures_ >= kWifiFailureLimit) {
    enterProvisioningMode();
  }
}
...
void Provisioning::ensureMdns() {
  ...
  MDNS.addServiceTxt("skyfeeder", "tcp", "step", "sf_step14B_buttonless_provision");
  mdns_ready_ = true;
}
...
void Provisioning::publishDiscovery(PubSubClient& client) {
  ...
  doc["step"] = "sf_step14B_buttonless_provision";
