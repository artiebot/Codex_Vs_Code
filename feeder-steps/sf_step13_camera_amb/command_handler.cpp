...
static void handleCamera(byte* payload,unsigned int len){
  ...
  char errmsg[32];
  bool ok=SF::amb.handleCommand(doc.as<JsonVariantConst>(), errmsg, sizeof(errmsg));
  StaticJsonDocument<160> ack;
  ack["cmd"]="camera";
  ack["ok"]=ok;
  ack["status"]=SF::amb.status();
  if(!ok) ack["msg"]=errmsg[0]?errmsg:"camera_fail";
  char buf[160]; size_t n=serializeJson(ack,buf,sizeof(buf)); (void)n;
  SF::mqtt.raw().publish(TOPIC_ACK, buf, false);
}
void SF_registerCommandSubscriptions(PubSubClient& client){ client.subscribe(TOPIC_CMD_LED,1); client.subscribe(TOPIC_CMD_CALIBRATE,1); client.subscribe(TOPIC_CMD_CAMERA,1); client.subscribe(TOPIC_CAMERA_AMB_EVENT_ANY,1); }
void SF_onMqttMessage(char* topic, byte* payload, unsigned int len){
  ...
  else if(strcmp(topic,TOPIC_CMD_CAMERA)==0) handleCamera(payload,len);
  else if(strncmp(topic, TOPIC_CAMERA_AMB_EVENT_PREFIX, strlen(TOPIC_CAMERA_AMB_EVENT_PREFIX))==0) SF::amb.onCameraEvent(topic,payload,len);
}