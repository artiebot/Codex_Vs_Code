...
void queueEvent(const char* state, const char* version, const char* fromVersion, const char* toVersion, const char* reason) {
  gEvent.active = true;
...
bool downloadAndStage(const ParsedCommand& cmd, PubSubClient& client, char* error, size_t errorLen) {
  ...
  while (http.connected()) {
    ...
    mbedtls_sha256_update_ret(&shaCtx, buffer, read);
...
void publishError(PubSubClient& client, const char* reason, const char* detail) {`r`n  publishEvent(client, "error", nullptr, detail ? detail : reason);`r`n}
