...
void Provisioning::begin(){ gProvisioning=this; pinMode(PROVISION_BUTTON_PIN, INPUT_PULLUP); Storage::begin(); load(); if(buttonRequestedSetup() || !ready_){ enterProvisioningMode(); } else { ready_=true; Topics::init(deviceId()); ensureMdns(); } }
...
void Provisioning::enterProvisioningMode(){ setup_mode_=true; ready_=false; Topics::init(deviceId()); startSetupAp(); runtime_press_active_=true; }
...
void Provisioning::publishDiscovery(PubSubClient& client){ if(discovery_published_||!ready_) return; StaticJsonDocument<512> doc; doc["device_id"]=deviceId(); doc["step"]="sf_step14_provision"; doc["services"][0]="weight"; doc["services"][1]="motion"; doc["services"][2]="visit"; doc["services"][3]="led"; doc["services"][4]="camera"; auto topics=doc["topics"].to<JsonObject>(); topics["status"]=Topics::status(); topics["ack"]=Topics::ack(); topics["telemetry"]=Topics::telemetry(); topics["cmd"] = Topics::cmdAny(); topics["event_visit"]=Topics::eventVisit(); topics["event_snapshot"]=Topics::eventCameraSnapshot(); char payload[512]; size_t n=serializeJson(doc,payload,sizeof(payload)); client.publish(Topics::discovery(), payload, true); discovery_published_=true; }
