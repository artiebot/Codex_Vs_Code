...
public final class SampleCaptureProvider: CaptureProvider {
    private let captures: [Capture]

    public init(referenceDate: Date = Date(), cacheManager: CacheManager = .shared) {
        var working: [Capture] = []
        ...
        for offset in 0..<8 {
            ...
            let thumbURL = (try? cacheManager.store(data: sampleThumbnailData, forKey: thumbKey, category: .thumbnails)) ?? cacheManager.url(forKey: thumbKey, category: .thumbnails)
            let assetURL = (try? cacheManager.store(data: sampleAssetData, forKey: assetKey, category: .assets)) ?? cacheManager.url(forKey: assetKey, category: .assets)
